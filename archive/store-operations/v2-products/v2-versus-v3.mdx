...
keywords: products, v2, catalog, v3, configurable_fields, options, modifiers
---
...
Every purchasable entity in the catalog is now a variant, including the product itself. This enables enhanced flows around inventory management, such as the ability to solely use the variants endpoint to manage inventory levels. For more on variants see [Variants](/docs/store-operations/catalog#variants).
...
Instead of a 422 error, now it will automatically copy the V2 global product option to a local product variant option or modifier option. This is triggered by an Update or Delete to either the [Product Variant Options](/docs/rest-catalog/product-variant-options#get-all-product-variant-options) or [Product Modifiers](/docs/rest-catalog/product-modifiers#get-all-product-modifiers) endpoints.
...
## Recommendations
When the resource is available in [V3](/docs/rest-catalog/products), it is best practice to use the V3 endpoint. For resources that do not have a V3 counterpart, like Orders, use V2. Both the V2 and V3 APIs authenticate with Oauth and are designed to be used concurrently within a single application.
...
They are not recommended for V3 products since they can be created at the variant level. See [Complex Rules](/docs/store-operations/catalog#complex-rules) for more information.
