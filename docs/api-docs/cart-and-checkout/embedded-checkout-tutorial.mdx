...
First, you need to use the JSON Web Token Standard to create a new token. Use a [JWT library (jwt.io)](https://jwt.io/#libraries-io) to accomplish this. For more information, see [Create JWT Using the Debugger Tool](/docs/start/authentication/customer-login#create-jwt-using-the-debugger-tool).
...
| `store_hash` | string | The subject store's store hash. |
| `customer_id` | integer | The ID of the customer who wants to checkout. |
| `channel_id` | integer | The sales channel that the customer is signing in to. In a headless implemention, this is the storefront's channel ID. | 
| `redirect_to` | string, URL | The cart redirect URL you generated in a previous step. |
| `request_ip` | string | Optional. The IP address the Customer Login API call will come from. |
