...
These rate limits enhance quality for merchants by regulating excessive API calls. In this document, we'll provide information on rate limits along with some best practices to put in place.
...
API rate limits regulate the amount of calls being made to your app. You can use the API's response HTTP headers to find more information about your current rate limit status:
