...
If you encounter a `429` response without an API HTTP header, this is an indication of the plaform undergoing a load balance to prevent any downtime from happening.
...
```http filename="Example: 429 response" showLineNumbers copy
 ...
 X-Rate-Limit-Time-Reset-Ms: 15000
```
Continue to reduce the risk of an app outage by slowing down the amount of calls being made. This will reduce any strain in performance.  For more details on response headers, see [About Our API](/docs/start/about#bigcommerce-specific-response-headers).
