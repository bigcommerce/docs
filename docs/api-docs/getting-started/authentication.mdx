...
To obtain a dynamic token, send a request to the REST endpoint listed in the **Obtain a JWT** column or consult the article listed in the **API Description** column. Use an API account with a [token creation scope](/docs/start/authentication/api-accounts#token-creation-scopes).
