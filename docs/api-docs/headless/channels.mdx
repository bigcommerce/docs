...
Currently, all checkout pages are hosted. Set the checkout domain for the headless storefront by sending a request to the [Upsert a site's checkout URL](/docs/rest-management/channels/site-checkout-url#upsert-a-sites-checkout-url) endpoint.
