...
* Note that you would not normally use a customer impersonation token in the GraphQL Storefront Playground. Using the GraphQL Storefront Playground and customer impersonation token are solely for this tutorial.
* Customer impersonation token-authenticated requests made to the GraphQL Storefront API can query store information from a customer's perspective by specifying the customer ID using the `x-bc-customer-id` header sent with the GraphQL POST request.
</Callout>
