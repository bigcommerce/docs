...
* You can make customer impersonation token-authenticated requests to the GraphQL Storefront API from a customer's perspective by specifying the customer ID using the `x-bc-customer-id` header in the GraphQL POST request.
</Callout>
