...
This end-to-end guide demonstrates how to power headless storefronts and other non-Stencil shopper experiences. These steps will allow you to build shopping experiences where the purchase funnel is entirely powered by GraphQL, from catalog browsing through checkout. The examples walk you through critical GraphQL capabilities to fit most use cases when creating a headless storefront:
...
Before starting, create a token allowing you to make GraphQL POST requests as a customer. A customer impersonation token permits you to perform actions as any shopper. 

<Callout type="info">
You could use a Storefront API Token to authenticate requests to the GraphQL Storefront API if you prefer to use expired tokens. For more on creating a Storefront API Token, see [Create a Token](https://developer.bigcommerce.com/docs/rest-authentication/tokens#create-a-token). 
</Callout>
...
<Callout type="info">
You can access Cart and Checkout details in the same request as other information.
</Callout>
...
Completing a checkout creates an incomplete order until payment is applied. For PCI compliance-related reasons, the GraphQL Storefront API does not handle payments. You can use the Payments API (Reference) to process payments. The payments workflow includes the following three steps:
...
### Related articles
* [creating a token](/docs/storefront/cart-checkout/guide/graphql-storefront#tokens)
* [querying product data](/docs/storefront/graphql#querying-within-a-bigcommerce-storefront)
* [creating a cart](/docs/storefront/cart-checkout/guide/graphql-storefront#create-a-cart-using-a-simple-product)
* [adding cart line items](/docs/storefront/cart-checkout/guide/graphql-storefront#add-cart-line-items)
* [getting a Checkout](/docs/rest-management/checkouts)
* [adding a checkout billing address](/docs/storefront/cart-checkout/guide/graphql-storefront#add-checkout-billing-address)
* [adding a checkout shipping consignment](/docs/storefront/cart-checkout/guide/graphql-storefront#add-a-checkout-shipping-consignment)
* [selecting shipping option](/docs/storefront/cart-checkout/guide/graphql-storefront#select-checkout-shipping-option)
* [completing a checkout](/docs/storefront/cart-checkout/guide/graphql-storefront#complete-checkout) 
