...
* Customer impersonation token-authenticated requests made to the GraphQL Storefront API can query store information from a customer's perspective by specifying the customer ID using the `x-bc-customer-id` header in the GraphQL POST request.
</Callout>
