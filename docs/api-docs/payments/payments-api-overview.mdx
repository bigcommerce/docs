...
You can process payments charged to either of two main forms of payment: [stored payment instruments](#stored-cards-and-paypal-accounts) or [new cards](#credit-cards), which still need to be saved. The API flow does not currently support hosted, offsite, or wallet-type providers like Amazon Pay. One exception here is that existing [stored PayPal accounts](https://support.bigcommerce.com/s/article/Enabling-Stored-Payment-Methods?language=en_US#paypal) are supported by only one provider, PayPal powered by Braintree. No providers support using non-stored/new PayPal accounts through this API.
## PCI compliance
