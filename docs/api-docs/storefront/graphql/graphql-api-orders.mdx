...
BigCommerce's GraphQL Storefront API Orders operation definition includes both Query and Mutation object types that Developers can use to create a custom order workflow.

Use the `order` operation to perform queries and mutations using your data, to select fields and relationships down to scalar values. Create arguments for any given fields like filtering orders by `entity_Id`.

This GraphQL API definition powers customer solutions as shoppers evolve from guests, to registered customers. Orders related mutations and queries can do the following:

### Get order details for a specific order

Get specific details for a specific order using the `order` query on the storefront graph under the site node.

Use the `Order` object type to:
