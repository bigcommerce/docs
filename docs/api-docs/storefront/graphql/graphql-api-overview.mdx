...
Use the [Create a GraphQL Storefront API token](/docs/rest-authentication/tokens#create-a-token) REST endpoints to request JWT bearer tokens that authenticate requests to the GraphQL Storefront API. Use a [token creation scope](/docs/start/authentication/api-accounts#token-creation-scopes) for the [store-level or app-level API account](/docs/start/authentication/api-accounts) in the request.
