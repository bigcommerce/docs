...
It's also possible to generate tokens for use in server-to-server or headless interactions with a trusted consumer using the [Create a customer impersonation token](/docs/rest-authentication/tokens/customer-impersonation-token) endpoint. Use the Storefront API Customer Impersonation Tokens [OAuth scope](/docs/rest-authentication/tokens/customer-impersonation-token#create-a-token) in the request.
