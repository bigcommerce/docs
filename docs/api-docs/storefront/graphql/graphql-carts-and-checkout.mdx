...
<Tabs items={['Request', 'Response']}>
  ...
  ```graphql filename="Example mutation: Create cart metafields" showLineNumbers copy
   type CartMutations {
  ...
  createCartMetafield(input: CreateCartMetafieldInput!): CreateCartMetafieldResult
}

input CreateCartMetafieldInput {
  cartEntityId: 27cb72e9-662c-424f-a1c3-317850c86004
  data: CreateCartMetafieldData!
}

input CreateCartMetafieldData {
  key: Color // required
  value: Red // required
}
...
```
  ...
  <Tab>
    ...
    ```json filename="Example mutation: Create cart metafields" showLineNumbers copy
