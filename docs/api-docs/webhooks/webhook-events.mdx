...
| store/sku/updated | Fires when you edit a variant globally or for a channel locale as overrides. The `context` fields are present for only override updates. |
