...
| Name / Scope             | Description |
|:-------------------------|:------------|
| store/modifier/updated | Fires when you edit attributes for a local or shared modifier. Updates to the global store and channel locale as overrides triggers the webhook. The `context` fields are present for only override updates.  |
...
| Name / Scope             | Description |
|:-------------------------|:------------|
| store/option/updated | Fires when you edit attributes for a local or shared variant option. Updates to the global store and channel locale as overrides triggers the webhook. The `context` fields are present for only override updates. |
...
| store/product/updated | Fires when you edit product attributes globally or for a channel locale as overrides. For a full list of product fields that trigger an updated event, see the [product updated events](#product-update-events) that follow. The `context` fields are present for only override updates. |
...
```json filename="Example product payload object" showLineNumbers copy
{
  ...
  "data": {
    ...
    "attributes": ["warranty", "is_featured"],
    "context": {
      "channel_id": 2,
      "locale": "fr"
    } 
...
| store/sku/updated | Fires when you edit the SKU of a product or variant globally or for a channel locale as overrides. The `context` fields are present for only override updates. |
...
```json filename="Example sku created payload object" showLineNumbers copy
{
  ...
  "data": {
    ...
    "attributes": ["cost_price", "is_free_shipping"],
    "context": {
      "channel_id": 2,
      "locale": "fr"
    },
...
| Name / Scope             | Description |
|:-------------------------|:------------|
| store/url-path/updated | Fires when you edit a product's URL path globally or for a channel locale as overrides. The `context` fields are present for only override updates. |

The payload object takes the form that follows:

```json filename="Example URL path payload object" showLineNumbers copy
{
  "scope": "store/url-path/updated",
  "store_id": "1025646",
  "data": {
    "entity_type": "sku", // will always be sku
    "entity_id": 461, // ID of the sku
    "context": {
      "channel_id": 2,
      "locale": "fr"
    }
