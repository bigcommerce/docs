...
| Name / Scope             | Description |
|:-------------------------|:------------|
| store/url-path/updated | Fires when you edit a product or variant's URL path globally or for a channel locale as overrides. The `context` fields are present for only override updates. |
...
```json filename="Example URL path payload object" showLineNumbers copy
{
  ...
  "data": {
    "entity_type": "product",
    "entity_id": 461, // ID of the product or variant
    "context": {
      "channel_id": 2,
      "locale": "fr"
    }
