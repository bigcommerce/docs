...
| **429** | **Too Many Requests** | When an OAuth client exceeds the [rate limit](/docs/api-docs/best_practices#best-practices_rate-limits) for API requests to a store. |
...
|**403**| API lacks required OAuth scopes, a store owner account changed, operations resulting from API request exceed a platform limit, or URL requested is incorrect.|Double-check OAuth Scopes. Check the URL. Are the endpoint and `authToken` correct? |
