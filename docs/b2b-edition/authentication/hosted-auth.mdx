...
```javascript filename="Fetch request example"
async function() {
  try {
...
```graphql filename="Example login response" showLineNumbers copy
{
    "data": {
        "login": {
            "result": {
...
```javascript filename="Fetch request example" showLineNumbers copy
const options = {
  method: 'GET',
...
```graphql filename="Example authorization response" showLineNumbers copy
{
    "data": {
        "authorization": {
            "result": {
...
To sync a login with the token, you must include a customer access token (CAT) in the request body. You can retrieve a CAT by using the `login` mutation of the [BigCommerce GraphQL Storefront API](https://developer.bigcommerce.com/docs/start/authentication/graphql-storefront#login-mutation), **not** the [equivalent request](#the-login-mutation) in the B2B Edition GraphQL Storefront API.

#### Get a Storefront API authToken

Rather than targeting a specific Company user, this endpoint retrieves an existing storefront authToken based on the JWT associated with an active storefront session. As with the [authorization mutation](#the-authorization-mutation), you can use the [Current Customer API](https://developer.bigcommerce.com/docs/storefront-auth/current-customer) to retrieve a JWT and pass it to the `jwtToken` query parameter of the request. Here are two examples of how you can format the request:
...
* [Authenticating requests to the GraphQL Storefront API](https://developer.bigcommerce.com/docs/start/authentication/graphql-storefront#login-mutation)
