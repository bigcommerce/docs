...
The `access_token` would naturally expire immediately after the user clicks the **Uninstall** button if no calls were made. If at least one call has been made, the `access_token` will expire precisely one hour after the user clicks the **Uninstall** button. This hour grace period is crucial as it would allow apps to remove any content they might have added to the customer's store, which they should no longer be able to access after unsubscribing from the app's services.
