...
* No Billing During Trial
  * A subscription only becomes active—and generates an invoice—after the trial period ends. No charges are made during the trial.

* Immediate Cancelation During Trial
   * If a subscription is cancelled while it's still in the trial period, the cancellation takes effect immediately. The trial ends, and no invoice is ever generated.

* Post-Trial Cancelation Behavior
   * Once the trial ends and the subscription begins billing, any cancellation initiated will take effect at the end of the current billing cycle (e.g., monthly or annual term), not immediately.
