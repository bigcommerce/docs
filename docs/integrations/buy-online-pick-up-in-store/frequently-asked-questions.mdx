...
Apps that are [compatible with multi-location inventory](https://support.bigcommerce.com/s/article/Multi-Location#app-support) must use the [Inventory API](/docs/store-operations/catalog/inventory-adjustments), not the [V3 Catalog API](/docs/rest-catalog), to adjust inventory in a store.