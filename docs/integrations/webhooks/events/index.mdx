...
| store/product/customField/updated |  Fires when you update a custom field for a product globally or for a channel locale as an override. The `context` fields are present for only updates to overrides, not the global store. For information on updating overrides, see the [International Enhancements for Multi-Storefront](/docs/store-operations/catalog/msf-international-enhancements) overview. |
...
* `store/product/customField_updated`

```json filename="Example payload object for a product custom field" showLineNumbers copy
{
  "scope": "store/product/customField/updated",
  "store_id": "1025646",
  "data": {
    "product_id": 1,
    "custom_field_id:": 205,
