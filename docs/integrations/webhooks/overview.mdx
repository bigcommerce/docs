...
* A maximum of 10 webhooks per unique combination of `store_id` + `channel_id` + `client_id` + `scope`.
  NOTE: Existing accounts exceeding this limit will continue to function without disruption. 

* Only one webhook is allowed per unique combination of `store_id` + `channel_id` + `client_id` + `scope` + `destination`.
