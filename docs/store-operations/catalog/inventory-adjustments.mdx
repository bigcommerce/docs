...
When you create a product, the inventory at each location is `undefined` under the hood, even if `available_stock` appears to be `0`. You must make an absolute adjustment to a product's inventory before you can make a relative adjustment.
