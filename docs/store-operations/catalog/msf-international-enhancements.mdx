...
The following pages provide example queries:
...
The following are deprecated for some features, when specifying a channel and locale:
- `localeContext`. Use `overrides` in the input instead.
- `overridesForLocale`. Use `overrides` in the query or mutation.

The example query pages contain the most up-to-date queries. Deprecated fields can be found in the [GraphQL Admin API reference](https://developer.bigcommerce.com/graphql-admin/reference).