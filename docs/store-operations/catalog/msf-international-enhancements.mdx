...
For some features, the following fields are deprecated, when specifying a channel and locale.
- `localeContext` 
- `overridesForLocale`

Use `overrides` in the input instead. The example query pages contain the most up-to-date queries. Deprecated fields can be found in the [GraphQL Admin API reference](https://developer.bigcommerce.com/graphql-admin/reference).
