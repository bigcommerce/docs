...
Create customer segments to offer customers targeted promotions. See the [Customer Segmentation overview](/docs/store-operations/customer-segmentation#introduction) to get the big picture on targeting customers with promotions. For more granular details, see the [Targeting a segment with a promotion](#targeting-a-segment-with-a-promotion) section of this article.
