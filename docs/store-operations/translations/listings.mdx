...
The response example below specifically references category translations. If your query is for brands, the `resourceId` will be different. Make sure your handling code addresses the correct format.
...
The request below is for updating a category. For brands, replace `resourceType` and `resourceId` with appropriate values from the [Resource fields](#resource-fields) table.
...
The request below is for deleting translations on a category. For brands, replace `resourceType` and `resourceId` with appropriate values from the [Resource fields](#resource-fields) table.
