...
<Callout type='info'>
	Order status IDs are predefined and static; merchants can only change the status name. See the [Order Status REST API](https://developer.bigcommerce.com/docs/rest-management/orders/order-status#get-all-order-statuses) for the full list of statuses and their IDs.
</Callout>
