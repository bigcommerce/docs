...
The hash value begins with at least one string, with each string including a prefix indicating a particular hash algorithm (currently the allowed prefixes are sha256, sha384, and sha512), followed by a dash, and ending with the actual base64-encoded hash.
...
```
<Callout type="info">
 If you are using a multi-storefront setup and have a channel specific custom checkout, navigate to <em>Channel Manager > Storefronts > Edit settings > Checkout</em> instead.
 </Callout>
