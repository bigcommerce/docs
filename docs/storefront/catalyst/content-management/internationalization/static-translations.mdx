...
The storefront theme, or fixed text elements within the storefront's shopper interface, can be translated into multiple languages using predefined static strings.
...
<Callout type="info">
**Default language**<br />
When you [set up your project](/docs/storefront/catalyst/getting-started), Catalyst hardcodes the default language file in US English. You can adjust the phrase strings in the resulting `en.json` file to meet your needs.
</Callout>
...
The JSON files should contain key-value pairs for each locale. You can define translations based on pre-defined keys used to translate the Catalyst storefront's [basic ecommerce functionality](/docs#ecommerce-functionality). The translated values you specify will display to shoppers as static string translations.
...
For example, if you created a `fr.json` file, include `fr` in the `localeLanguageRegionMap` array if it doesn't exist there.
...
Even though the next-intl library supports several [locale detection strategies](https://next-intl-docs.vercel.app/docs/routing/middleware#strategies), Catalyst doesn't use any by default. Strategy can be changed in the `routing.ts` config file.
