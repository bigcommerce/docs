...
We recommend using the CLI to create a new Catalyst project. The CLI will guide you through the process of creating a new project and setting up your development environment. If you prefer to create a project manually, you can follow the manual installation instructions. For a guide to manually create a Catalyst channel with the REST Management API and also covers setting up routes for your channel and connecting a Makeswift site, see [Manually create a Catalyst channel](/docs/storefront/catalyst/getting-started/workflows/manually-create-catalyst-channel).
