...
Although path-to-entity mappings can be cached, Next.js middleware cannot use the built-in `cache()` API. Instead, a separate caching backend must be used. Catalyst uses a [caching implementation](https://github.com/bigcommerce/catalyst/tree/main/core/lib/kv) that’s flexible for multiple KV adapters.
