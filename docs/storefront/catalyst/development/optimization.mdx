...
## Vercel Runtime Cache for Route Caching
...
can be resolved into the URL pattern expected by the appropriate Next.js route, such as a product path with an ID:

```
/product/123
```

Resolving the path requires an additional blocking API request per page. To avoid unnecessary requests and maintain optimal performance, the resulting data should be cached.
