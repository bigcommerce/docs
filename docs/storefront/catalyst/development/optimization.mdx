...
Catalyst makes use of the Next.js Data Cache to avoid unnecessary duplicate API requests, and you should take advantage of this pattern where feasible in your own custom requests. See [Caching](/caching) for more details.
...
For caching route data in the Next.js middleware layer, Catalyst will automatically make use of a KV storage solution if it is configured in your environment. See [Middleware](/catalyst-middleware) for more details.  Any Catalyst production deployment should have KV storage configured.
