...
| Express Wallet Buttons | 🟡 | Not directly supported in Catalyst (for example, on the Cart page). Express wallet buttons do work on Checkout, which is rendered by Stencil rather than Catalyst. The support boundary for this and many features falls between the Cart page (Catalyst) and the Checkout page (Stencil). |
