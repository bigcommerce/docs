...
- `export const middleware` has been renamed to `export const proxy`
- The internal `middlewares/` directory has been renamed to `proxies/`
...
4. Rename `middleware.ts` to `proxy.ts` and change `export const middleware` to `export const proxy`.
5. Rename the `middlewares/` directory to `proxies/` and update any internal imports accordingly.
6. Ensure you are running **Node.js 24 or later**.
7. Address any Next.js 16 deprecation lint errors (e.g., legacy `<img>` elements, missing `rel="noopener noreferrer"` on external links).
...
- **Makeswift layout shift**: Upgraded `@makeswift/runtime` to `^0.26.3`, which replaces `<Suspense>` with the React `<Activity>` API to eliminate layout shift when loading Makeswift pages and when client-side navigating between Makeswift and non-Makeswift pages.
