...
A fundamental concept in BigCommerce analytics is the "visit" or "session". When sending events to BigCommerce you must include a `visitId` and `visitorId`, both formatted as UUID. You can generate these values using the [UUID Generator](https://www.uuidgenerator.net/version4).
...
Execute the `VisitStartedEvent` mutation. You must provide the `visitId` and `visitorId` (in UUID format) in the `initiator` input. 
...
Execute the `ProductViewedEvent` mutation, using the same `visitId` and `visitorId` as in the `VisitStartedEvent`. This step links the product view to the session.
...
Navigate to the `redirectedCheckoutUrl` returned in the previous step, proceed through checkout, and create an order.
</Steps>
