...
Use `visitId` and `visitorId` (UUID format) in the `initiator` input. Specify `refererUrl: "https://google.com"` in the `request` input.
...
Use the same `visitId` and `visitorId` as in the `VisitStartedEvent`.
...
After added a product to cart and obtaining a `cart_id`, execute the `redirect` mutation. Use the same `visitId` and `visitorId` as in the `VisitStartedEvent` and the `cart_id`. You will generate a `redirectedCheckoutUrl`. Proceed through checkout and create an order.
