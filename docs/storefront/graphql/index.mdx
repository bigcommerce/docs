...
The GraphQL Storefront API allows bi-directional pagination. The following scenarios are available:

* Base query
* Forward pagination
* Backward pagination

**Base Query**: You can use a query as the base case for the next queries. The example below shows a query for 50 products.
...
**Forward pagination**: You can query in a forward direction using the `hasNextPage=true` and the `hasPreviousPage=false`. The example below queries the first 5 products.
...
**Backward pagination**: You can query in a backward direction using the `hasNextPage=false` and the `hasPreviousPage=true`. The example below queries the last 5 products.
