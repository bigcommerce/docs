...
- Inspect `X-BC-Rate-Limit-Reason-Code` first to distinguish requests not limited (`0`) from requests throttled by the per-IP inflight complexity limiter (`1`).
- Inspect `X-BC-IP-Rate-Limit-Requests-Quota` for the configured quota value.
- Inspect `X-BC-IP-Rate-Limit-Requests-Quota-Left`: to understand whether you have enough headroom or are close to the threshold.
