...
To make queries from the perspective of a particular customer in headless or server-side code, use [customer access tokens](/docs/start/authentication/graphql-storefront#customer-access-tokens) rather than storefront tokens. 

Depending on your implementation, you can use the login mutation, the [Current Customer API](/docs/start/authentication/current-customer), or the REST Management API's [customers feature](/docs/rest-management/customers) to authenticate the customer and get their customer ID. Pass the customer ID with your GraphQL Storefront requests using the `X-Bc-Customer-Access-Token` header to interact with the store from the customer's perspective. The responses will reflect customer-specific pricing, product availability, account information, and more.

## Customer single sign-on

When a customer signs in to your headless storefront and your application redirects them to a BigCommerce-hosted page, such as checkout, you can use a Storefront [token](/docs/rest-authentication/tokens#create-a-token) to sign them in to the hosted session. 

You can sign a customer in to an embedded checkout by setting the GraphQL Storefront API [createCartRedirectUrls mutation](/graphql-storefront/reference#definition-CartMutations) to the relative path of the embedded checkout URL.
