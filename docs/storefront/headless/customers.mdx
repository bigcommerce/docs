...
To make queries from the perspective of a particular customer using headless or server-side code, use [customer access tokens](/docs/start/authentication/graphql-storefront#customer-access-tokens) and regular [storefront tokens](/docs/start/authentication/graphql-storefront#storefront-tokens). Then, assign `X-Bc-Customer-Access-Token` to the customer access token in the header.

Depending on your implementation, you can also use the [Current Customer API](/docs/start/authentication/current-customer) and the REST Management API's [customers feature](/docs/rest-management/customers) to authenticate the customer and get their customer ID. Pass the customer ID with your GraphQL Storefront requests using the `X-Bc-Customer-Id` header to interact with the store from the customer's perspective. The responses will reflect customer-specific pricing, product availability, account information, and more. 
