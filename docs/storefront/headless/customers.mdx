...
One key use of the session sync token is to enable smooth redirection to the checkout page. This works for both logged-in customers and anonymous shoppers. To support this, the token can include a `redirect_to` field in its payload. After validating the token, this field is used to redirect the shopper to the appropriate page (e.g., checkout), carrying over all necessary session context.
