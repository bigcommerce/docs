...
<Callout type="info">
Typically, there is no need to use a customer impersonation token in the GraphQL Storefront API playground. Using the playground with a customer impersonation token is solely an example in this guide. You can use a customer access token, which is the preferred choice.
</Callout>
