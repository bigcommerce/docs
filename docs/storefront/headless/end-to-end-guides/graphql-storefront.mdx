...
Next, you will need to assign the customer access token to `X-Bc-Customer-Access-Token` in the header. 

NOTE: The token is invalidated when you use the logout mutation with the `X-Bc-Customer-Access-Token header`.
