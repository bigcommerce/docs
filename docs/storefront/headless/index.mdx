...
The GraphQL API now supports passing a visitor ID in session sync mutations, enhancing tracking consistency across different contexts. The visitor ID acts as a unique identifier that links a user's activity across multiple sessions, ensuring that analytics session IDs remain persistent as users move across various parts of the storefront and other digital touchpoints.
