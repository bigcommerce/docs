...
| X-Forwarded-For | Original IP address of the remote user (shopper's browser or backend service) | Proxy server |
...
2. Configure your proxy server to:
   ...
   - Pass the remote user's IP in the `X-Forwarded-For` header
