...
<details>
<summary> Click to see the full list of supported languages.
</summary>
...
If a translation is missing for a product’s content in the shopper’s selected language, the API automatically returns the “base” content (store’s default language) for those attributes, ensuring products remain discoverable even when translations are incomplete.
...
- [Store Search Product Fields](https://support.bigcommerce.com/s/article/Store-Search?language=en_US#best-practices)
